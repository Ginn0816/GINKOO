...
public class Page0511103Cust extends Page0511103 {
	...
	protected void beforeRender() {
		...
		// 日期赋值示例 map.put("WORK_DATE",
		// DateUtils.parseDate(ContextHelper.getContext().getValue("WORK_DATE")));
	...
	protected void bindEvent() {
		BTN_INQ_Listener = new ClickListener() {
			...
			public void onClick(ClickEvent event) {
				...
				// 对于通用查询需要设置SQLID的值
				BTN_INQ_Cmd.setQuerySqlid("051110301");
				...
				SUBWINDOW_Cmd.execute(new CommandCallback() {
					...
					public void onCallback(boolean status, Callback callback,
					 ...
					 // 查询子列表
						...
						if (inputDs.getCurrentValue("PBOC_STATUS").equals(
								"RJCT")
								|| inputDs.getCurrentValue("PBOC_STATUS")
										.equals("EDRN")) {
							frmJYZTXX1.show();
						} else {
							frmJYZTXX1.hide();
		...
		resultTb2_RecordClickListener = new RecordClickListener() {
        ...
        this.resultTb2.addRecordClickListener(resultTb2_RecordClickListener);
		
		ClickListener = new ClickListener() {
			@Override
			public void onClick(ClickEvent event) {
				inputDs.setCurrentValue("TXN_SEQ_NO",
						resultDs.getCurrentValue("TXN_SEQ_NO"));
				// TODO 按钮点击事件
