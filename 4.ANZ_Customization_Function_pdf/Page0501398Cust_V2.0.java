...
//            		System.out.println("payerNameAddr len[" + nameAddr + "] [" + nameAddr + "]");
            		nameAddr = nameAddr.replaceAll("\r|\n|\t", " ");
            		if (nameAddr.length() > 140) {
//            			UPrompt.alert("提示", "付款人户名+地址长度超长，当前长度[" + nameAddr.length() + "] 多出 [" + (nameAddr.length()-140) + "]个字符");
//            			return;
...
//            			nameAddr = nameAddr + " ADD. " + payeeAddr1;
...
//            		System.out.println("payeeNameAddr len[" + nameAddr + "] [" + nameAddr + "]");
            		nameAddr = nameAddr.replaceAll("\r|\n|\t", " ");
            		if (nameAddr.length() > 140) {
//            			UPrompt.alert("提示", "收款人户名+地址长度超长，当前长度[" + nameAddr.length() + "] 多出 [" + (nameAddr.length()-140) + "]个字符");
//            			return;
            		...
            		if (ps1.length() > 140) {
//            			UPrompt.alert("提示", "附言1长度超长，当前长度[" + ps1.length() + "] 多出 [" + (ps1.length()-140) + "]个字符");
//            			return;
...
//            		System.out.println("PS=" + tmp72);
            		if (len + line * 2 > 35 * 6) {
//            			UPrompt.alert("提示", "附言2,3,4长度超长，当前长度[" + (len + line * 2) + "] 多出 [" + ((len + line * 2)-35*6) + "]个字符");
//            			return;
...
//            	if (CIPSCommon.isDataNull(inputDs, "PAYEE_BRNO") && CIPSCommon.isDataNull(inputDs, "PAYEE_BRNAME")) {
//            		UPrompt.alert("提示信息", "收款间参行为空！");
//					return;
//            	}
...
//            	if (inputDs.getCurrentValue("CHK_RESULT").contains("黑名单命中") && isMod == "0") {
//					confirm(inputDs.getCurrentValue("CHK_RESULT"),inputDs.getCurrentValue("CNMS_SEQ_NO"),false);
//				} else {
//					executeBtnOkCmd();
//				}
...
//		this.BTN_SUB_RECHK.addClickListener(new ClickListener() {
//			
//			@Override
//			public void onClick(ClickEvent event) {
//				// TODO Auto-generated method stub
